...
export class EditBusComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    if (!this.id) {
      this.toast.error('Invalid Bus ID');
  ...
  loadBus(): void {
    ...
    this.busService.getBusById(this.id).subscribe({
      ...
      error: (error) => {
        ...
        this.toast.error('Failed to Load Bus');
    ...
    this.busService.updateBus(this.id, this.busForm.value).subscribe({
      ...
      error: (error) => {
        ...
        if (error.status === 400) {
          ...
          this.toast.error('Invalid Bus Details');

        } else if (error.status === 404) {

          this.toast.error('Bus Not Found');
          ...
          this.toast.error('Failed to Update Bus');
